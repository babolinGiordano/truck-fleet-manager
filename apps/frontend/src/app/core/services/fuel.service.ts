...
import { Fuel, FuelType } from '../../models';
...
export class FuelService {
  ...
  createFuelRecord(record: Omit<Fuel, 'id' | 'createdAt' | 'updatedAt'>): Observable<Fuel> {
    return this.http.post<Fuel>(this.apiUrl, record).pipe(
      tap(created => {
  ...
  updateFuelRecord(id: string, updates: Partial<Fuel>): Observable<Fuel> {
    const { id: _id, createdAt, updatedAt, ...data } = updates as Fuel;

    return this.http.patch<Fuel>(`${this.apiUrl}/${id}`, data).pipe(
      tap(updated => {
