...
import { Maintenance, MaintenanceStatus, MaintenanceType } from '../../models';
...
export class MaintenanceService {
  ...
  createMaintenanceRecord(record: Omit<Maintenance, 'id' | 'createdAt' | 'updatedAt'>): Observable<Maintenance> {
    return this.http.post<Maintenance>(this.apiUrl, record).pipe(
      tap(created => {
  ...
  updateMaintenanceRecord(id: string, updates: Partial<Maintenance>): Observable<Maintenance> {
    const { id: _id, createdAt, updatedAt, ...data } = updates as Maintenance;

    return this.http.patch<Maintenance>(`${this.apiUrl}/${id}`, data).pipe(
      tap(updated => {
